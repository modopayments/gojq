...
// TermTypeFromString returns the TermType for the given name string, or 0 if unknown.
...
func (termType TermType) MarshalJSON() ([]byte, error) {
	...
	return json.Marshal(termType.GoString()[5:]) // strip "gojq." prefix
}
...
func (termType *TermType) UnmarshalJSON(text []byte) error {
	...
	*termType = TermTypeFromString(s)
	if *termType == 0 {
		return fmt.Errorf("unknown term type %v", s)
	}
	return nil
}
